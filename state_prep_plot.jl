...
function plot_entropy6(args)
    ...
    for i in rounds
        ...
        plot!(p_list,[df[j,"avg_entropy_$i"] for p in p_list for j in 1:length(df[!,"avg_entropy_$i"]) if df[j,"p"]==p],yerr=sem,label = "q=$i",xlabel = L"p_{in}",ylabel=L"\langle S(\rho) \rangle /n",marker=(:circle,4), grid=false, legend=:topleft,  xtickfontsize=15,ytickfontsize=16,xguidefontsize=18,yguidefontsize=18,legendfontsize=14, linewidth=2,xticks=([0.01,0.02,0.03],[0.01,0.02,0.03]))
